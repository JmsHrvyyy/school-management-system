...
const SetupPassword = () => {
  ...
  // --- DAGDAG NA STATE PARA SA REDIRECT LOGIC ---
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        // --- DITO NATIN KUKUNIN KUNG SINO YUNG NAG-VERIFY ---
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4 font-sans">
      {/* ... (Yung Form UI mo na same as before) ... */}
      <div className="max-w-md w-full">
         {/* Copy-paste mo lang dito yung form ui na nasa previous code mo */}
         <div className="text-center mb-8">
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-white mx-auto mb-4 shadow-xl" style={{ backgroundColor: branding.theme_color || '#2563eb' }}><ShieldCheck size={32} /></div>
         ...
         <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 p-8">
            <form className="space-y-6" onSubmit={handleSubmit}>
               <div className="space-y-1.5">
                  ...
                  <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} required className="block w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:border-blue-500 transition-all text-sm font-bold" placeholder="Minimum 6 characters" />
               </div>
               <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Confirm Password</label>
                  <input type={showPassword ? "text" : "password"} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required className="block w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:border-blue-500 transition-all text-sm font-bold" placeholder="Repeat password" />
               </div>
               {error && <div className="text-red-500 text-xs font-bold text-center">⚠️ {error}</div>}
               <button type="submit" disabled={loading} className="w-full text-white font-black py-4 rounded-2xl shadow-xl" style={{ backgroundColor: branding.theme_color || '#2563eb' }}>
